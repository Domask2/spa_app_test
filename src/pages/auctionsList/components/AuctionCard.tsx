...
export function AuctionCard({ auction }: AuctionCardProps) {
    ...
    return (
        <div className="flex flex-col justify-between h-full bg-white p-4 rounded shadow hover:shadow-lg transition-shadow space-y-2 overflow-hidden border border-gray-100">
            ...
            >
                <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-start gap-2 transition-colors duration-200 group">
                        <span className="font-semibold text-lg truncate group-hover:text-blue-600 transition-colors duration-200">
                            {auction.cargoNum}
                        </span>
                        <div className="flex gap-2 flex-wrap flex-shrink-0">
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-xs whitespace-nowrap group-hover:bg-blue-200 transition-colors duration-200">
                                {auction.type}
                            </span>

                            <span className={`px-2 py-0.5 rounded text-xs whitespace-nowrap ${color} group-hover:ring-2 group-hover:ring-blue-300 transition-all duration-200`}>
                                {auction.status}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="text-sm grid grid-cols-2 gap-x-2 gap-y-0.5">
                        ...
                        <div className="truncate"><span className="text-gray-600">Вес:</span> {auction.cargo.weight} кг</div>
                        <div className="truncate"><span className="text-gray-600">Объём:</span> {auction.cargo.volume} м³</div>
                        <div className="truncate"><span className="text-gray-600">Кузов:</span> {auction.cargo.bodyType}</div>
                    </div>

                    {/* Цены и шаг */}
                    <div className="flex flex-wrap gap-3 text-sm font-medium">
                        <div className="truncate"><span className="text-gray-600">Цена:</span> {formatCurrency(auction.currentPrice)} ₽</div>
                        <div className="truncate"><span className="text-gray-600">За км:</span> {formatCurrency(auction.pricePerKm)} ₽</div>
                        <div className="truncate"><span className="text-gray-600">Шаг:</span> {formatCurrency(auction.betStep)} ₽</div>
                    </div>

                    {/* Подсказка при наведении */}
                    <div className="text-xs text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1">
